// ─────────────────────────────────────────────────────────────────────────────
// services/email.js — Full Email Notification Service (Nodemailer)
// ─────────────────────────────────────────────────────────────────────────────
...
const FROM = process.env.SMTP_FROM || process.env.SMTP_USER || 'cloudguard@example.com';
...
async function send({ to, subject, html, text }) {
  ...
  if (!recipients) return { skipped: true, reason: 'No recipient' };

  const info = await t.sendMail({ from: FROM, to: recipients, subject, html, text });
...
function wrap(title, body, severity = 'info') {
  const colors = {
    critical: '#ff4747', high: '#ffb547', medium: '#47c8ff', info: '#c6f135', success: '#c6f135'
  };
...
async function sendSecurityAlert({ accountId, issues = [], to }) {
  ...
  const body = `
    <div class="alert-box">
      Found <strong>${criticalCount} critical</strong> and <strong>${highCount} high</strong> severity issues in account <code>${accountId}</code> that require immediate attention.
    </div>
    <table class="data">
      <thead><tr><th>Severity</th><th>Resource</th><th>Issue</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
    <p style="color:#555;font-size:12px;margin-top:8px">Showing top ${Math.min(issues.length,15)} of ${issues.length} issues. Log in to CloudGuard to see all and auto-fix.</p>
  `;
...
async function sendCostAlert({ accountId, currentCost, forecastedCost, threshold, to }) {
  const body = `
    ...
    <p style="color:#777;font-size:13px">Review your service breakdown in CloudGuard and consider rightsizing or stopping idle resources.</p>
  `;
...
async function sendAbsenceAlert({ accountId, userId, daysMissing, servicesWillStop, to }) {
  const body = `
    <div class="alert-box">
      User <strong>${userId}</strong> has been inactive for <strong>${daysMissing} days</strong> in account <code>${accountId}</code>. 
      ${daysMissing >= 5 ? `<br><br>⚠️ <strong>AWS services are being automatically stopped to prevent cost overrun.</strong>` : `CloudGuard will stop services if inactivity continues for ${5 - daysMissing} more days.`}
    </div>
    ${servicesWillStop?.length ? `
    <table class="data">
      <thead><tr><th>Resource</th><th>Type</th><th>Status</th></tr></thead>
      <tbody>
        ${servicesWillStop.map(s => `<tr><td style="font-family:monospace">${s.id}</td><td>${s.type}</td><td><span class="badge ${daysMissing >= 5 ? 'badge-critical' : 'badge-high'}">${daysMissing >= 5 ? 'STOPPING' : 'AT RISK'}</span></td></tr>`).join('')}
      </tbody>
    </table>` : ''}
  ...
  return send({
    to,
    subject: `⏸ [CloudGuard] ${daysMissing >= 5 ? 'SERVICES STOPPING' : 'Absence Warning'} — ${userId} (${daysMissing}d inactive)`,
    html: wrap(`User Absence Detected: <span>${daysMissing} Days</span>`, body, daysMissing >= 5 ? 'critical' : 'high'),
    text: `CloudGuard: ${userId} has been inactive ${daysMissing} days in ${accountId}. ${daysMissing >= 5 ? 'Services being stopped.' : `${5-daysMissing} more days until auto-stop.`}`
  });
}
...
async function sendAbsencePlan({ accountId, userId, absencePlan, to }) {
  ...
  const body = `
    ...
    <p style="font-size:12px;color:#555;margin-top:8px">All actions are automated. Login to CloudGuard to modify or cancel this plan.</p>
  `;
...
async function sendWeeklySummary({ accountId, summary, to }) {
  const body = `
    ...
    <table class="data" style="margin-top:16px">
      ...
      <tbody>
        <tr><td>Security Score</td><td style="color:#c6f135;font-family:monospace;font-weight:700">${summary.securityScore}/100</td><td style="color:${summary.scoreChange >= 0 ? '#c6f135' : '#ff4747'};font-family:monospace">${summary.scoreChange >= 0 ? '+' : ''}${summary.scoreChange}</td></tr>
        <tr><td>Critical Issues</td><td style="color:#ff4747;font-family:monospace">${summary.criticalIssues}</td><td style="color:#777;font-family:monospace">${summary.criticalChange >= 0 ? '+' : ''}${summary.criticalChange}</td></tr>
        <tr><td>Monthly Spend</td><td style="color:#ffb547;font-family:monospace">$${summary.monthlyCost?.toFixed(2)}</td><td style="color:${summary.costChange <= 0 ? '#c6f135' : '#ffb547'};font-family:monospace">${summary.costChange >= 0 ? '+' : ''}${summary.costChange?.toFixed(1)}%</td></tr>
        <tr><td>Auto-Fixes Applied</td><td style="color:#c6f135;font-family:monospace">${summary.autoFixes}</td><td></td></tr>
      </tbody>
    </table>
  `;
